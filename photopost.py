...
from libs import config, publishers
...
def collect(picture=None):
    """Prompt for the picture and its metadata."""
    ...
    post.date = ask("Date of capture? (leave blank if not) ")

    post.location = ask("Place of capture? (leave blank if not) ")
    if post.location:
        post.lat = ask("Latitude ? ")
        post.lng = ask("Longitude ? ")
